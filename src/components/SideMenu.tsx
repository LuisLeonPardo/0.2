...
const SideMenu: React.FC = () => {
  ...
  const menuItems = [
    { name: 'Home', icon: Home, path: '/', description: 'Página principal' },
    { name: 'Nuevo Producto', icon: Plus, path: '/new-product', description: 'Agregar producto' },
    { name: 'Inventario', icon: Archive, path: '/inventory', description: 'Ver inventario' },
    { name: 'Smart Inventory', icon: Brain, path: '/smart-inventory', description: 'IA para inventario' }
  ];
  ...
  return (
    <div className="hidden md:flex md:flex-col md:fixed md:inset-y-0 md:left-0 md:w-64 md:bg-bg-surface md:border-r md:border-divider md:z-40">
      ...
      <div className="flex items-center h-16 px-6 border-b border-divider">
        ...
        <span className="text-xl font-bold text-text-primary">Inventory Pro</span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                <div className={`font-medium ${isActive ? 'text-complement-700' : 'text-text-primary'}`}>
                  {item.name}
                </div>
        ...
        >
          <div
            className={`flex items-center space-x-3 p-3 rounded-lg transition-all duration-300 ease-in-out ${
              showLogout ? 'bg-error-50 border border-error-200 shadow-sm' : 'hover:bg-gray-50'
            }`}
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-text-secondary truncate">Administrador</div>
            </div>
            <div
              className={`transition-all duration-300 ease-in-out ${
                showLogout ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'
              }`}
            >
